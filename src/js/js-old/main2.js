...
$(window).scroll(function() {
    if ($(this).scrollTop() > 870) {
        ...
        $('.md-services').addClass('activeAnim');
    } else {

        $('.md-services').removeClass('activeAnim');
    }
    if ($(this).scrollTop() > 2600) {
        // console.log($(this).scrollTop())
        $('.md-clients').addClass('activeAnim');
    } else {

        $('.md-clients').removeClass('activeAnim');
    }
});
/* =========================================================
 fixHeightNotif
...
if (jQuery('.md-job-details-form').length) {
    ...
    $('#jobForm').formValidation({
        ...
        // $(e.target).data('formValidation')
        e.preventDefault();
        $(e.target).data('formValidation').resetForm(true);
        $('.fileUpload span').html('Pièce jointe *');
        $('.alert-cutom').slideDown('400').css('opacity', '1');
        $('.close').on('click', function(event) {
            event.preventDefault();
            $('.alert-cutom').slideUp('400').css('opacity', '0');
        });
    })
    /* ==============uploadBtn=================== */
